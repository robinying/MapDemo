package com.robin.commonUi.customview.guide;
...
class Calculator {
    ...
    Calculator(Activity activity, FocusShape focusShape, View view, double radiusFactor,
               boolean fitSystemWindows, int adjustHeight, int offsetX) {
        ...
        mBitmapHeight = deviceHeight - (fitSystemWindows ? 0 : GuideUtils.getStatusBarHeight(activity));
        if (view != null) {
            if (adjustHeight == -1) {
                adjustHeight = (fitSystemWindows && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
                        ? 0 : GuideUtils.getStatusBarHeight(activity));
            }
            ...
            mCircleCenterX = viewPoint[0] + mFocusWidth / 2 + offsetX;
