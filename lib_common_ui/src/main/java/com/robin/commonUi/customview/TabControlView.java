...
import com.robin.commonUi.util.ThemeUtils;
...
public class TabControlView extends RadioGroup {
     ...
     */
    private RadioGroup.OnCheckedChangeListener mSelectionChangedListener = new OnCheckedChangeListener() {
        @Override
    ...
    private void initAttrs(Context context, AttributeSet attrs) throws Exception {
        ...
        try {
            ...
            mSelectedDrawable = attributes.getDrawable(R.styleable.TabControlView_tcv_selectedDrawable);
            mUnSelectedDrawable = attributes.getDrawable(R.styleable.TabControlView_tcv_unselectedDrawable);
    ...
    private void updateRadioButton(RadioButton rb, int tcv_option_normal, int tcv_option_selected) {
        GradientDrawable unselected;
        if (mUnSelectedDrawable == null) {
            unselected = (GradientDrawable) mContext.getResources().getDrawable(tcv_option_normal).mutate();
            unselected.setStroke(mStrokeWidth, mSelectedColor);
            unselected.setDither(true);
            unselected.setColor(mUnselectedColor);
        } else {
            unselected = (GradientDrawable) mUnSelectedDrawable.mutate();
        }
        GradientDrawable selected;
        if (mSelectedDrawable == null) {
            selected = (GradientDrawable) mContext.getResources().getDrawable(tcv_option_selected).mutate();
            selected.setColor(mSelectedColor);
            selected.setStroke(mStrokeWidth, mSelectedColor);
        } else {
            selected = (GradientDrawable) mSelectedDrawable.mutate();
        }
